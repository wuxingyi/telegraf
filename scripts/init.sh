...
# Default-Stop:      0 1 6
# Short-Description: Start the Telegraf process
### END INIT INFO

# If you modify this, please make sure to also edit telegraf.service
...
function start() {
    ...
    if which start-stop-daemon &>/dev/null; then
        ...
        local CMD="$DAEMON -pidfile $PIDFILE -config $CONFIG $TELEGRAF_OPTS >>$STDOUT 2>>$STDERR &"
